  ...
  useEffect(() => {
    if (isOpen) {
      fetchUsers()
        ...
        .catch((err) => {
          console.warn("fetchUsers error in UserSwitchModal:", err);
        });
    }
  }, [isOpen]);
        ...
        <div className="p-5 bg-gradient-to-r from-purple-600 to-pink-500 text-white flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-purple-100">เลือกบัญชีผู้ใช้งานจริงที่มีอยู่ในฐานข้อมูล</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-3 max-h-[70vh] overflow-y-auto">
          {usersList.length === 0 ? (
            <div className="p-6 text-center text-xs text-gray-500 bg-gray-50 rounded-xl border border-gray-200">
              ยังไม่มีบัญชีผู้ใช้งานในระบบ สามารถเพิ่มได้ที่เมนู "จัดการผู้ใช้และฐานข้อมูล"
            </div>
          ) : (
            usersList.map((u) => {
